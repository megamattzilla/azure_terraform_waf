...
begin
  ...
  JUICESHOP_DNS   = params['juice_shop_ips']['value']
rescue
  BIGIP_DNS       = []
  BIGIP_PORT      = ""
  BIGIP_PASSWORD  = ""
...
control "BIG-IP Licensed" do
  impact 1.0
  title "BIGIP has a valid license"

  BIGIP_DNS.each do |bigip_host|
    describe json(content: http("https://#{bigip_host}:#{BIGIP_PORT}/mgmt/shared/declarative-onboarding/task",
              auth: {user: 'admin', pass: BIGIP_PASSWORD},
              params: {format: 'html'},
              method: 'GET',
              ssl_verify: false).body) do
          its([0, 'result','status']) { should eq 'OK' }
    end
  end
end 